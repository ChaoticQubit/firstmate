...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr` is an experimental spawn backend (see [`docs/herdr-backend.md`](herdr-backend.md)).
Treehouse is the worktree provider for both, since herdr is a session provider only.
...
If both runtime markers are present, detection resolves innermost-first: `$TMUX` is checked before `HERDR_ENV=1`, since a tmux pane started from inside a herdr pane is the innermost, currently-executing layer.
...
`fm-spawn.sh` accepts `tmux` and `herdr` for ship, scout, and secondmate spawns alike.
...
Today that probe can classify tmux and herdr secondmate endpoints as `alive`, `dead`, or `unknown`; any other backend reports `unknown` until its own agent-process classifier is verified.
...
`fm-teardown.sh <id>` takes a task id directly and uses the same recorded `window=` backend target after loading `state/<id>.meta`.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux` or `herdr`), `jq` for the JSON-emitting experimental adapter (`herdr`) whose spawn and liveness paths parse the backend's JSON output, and the `treehouse` worktree provider, which both backends use.
An unknown resolved backend emits `BACKEND_INVALID` and blocks dispatch instead of silently dropping its dependency delta or falling back to tmux.
A herdr home is therefore never told `tmux` is missing.
...
FM_BACKEND=             # optional runtime backend override for new spawns; tmux/herdr support ship/scout/secondmate spawns, codex-app is not accepted
