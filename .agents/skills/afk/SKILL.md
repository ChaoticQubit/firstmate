...
- **Auto-discovered supervisor pane** - the daemon resolves its own BACKEND
  ...
  startup so a wrong-but-resolving fallback is detectable. Any other runtime
  backend is not yet supported as a supervisor backend; the daemon refuses
  loudly at startup instead of
