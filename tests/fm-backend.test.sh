...
# inside (a real tmux pane is the normal case for a captain's session).
...
# are both absent, selects between the markers exactly as fm_backend_detect
...
test_backend_validate_refuses_unknown() {
  ...
  local out
  # bogus names a backend with no adapter at all; tmux and herdr are both
  # known adapters and spawn-supported.
