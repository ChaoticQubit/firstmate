...
`docs/configuration.md` is the single owner of the operational-home layout, configuration schemas, and the state map, while each producing script's header and help own its exact fields and mutation mechanics.
Read those when you need one file's contract; the full tree is deliberately not duplicated here.
...
- `data/captain.md` holds this home's captain preferences, `data/captain-shared.md` the main-authoritative preferences inherited by secondmates, and `data/learnings.md` curated home-local knowledge; all three stay canonical even when harness memory mirrors them.
- `data/<id>/brief.md` carries a task's instructions, and `data/<id>/report.md` is a scout's deliverable that survives cleanup.

## 3. Session start (run once at every session start)

Run `bin/fm-session-start.sh` exactly once at session start.
Its header is the single owner of composed commands, ordering, digest contents, and emitted supervision instructions, so do not reimplement it by separately running its lock, bootstrap, or wake-drain parts.

Read the resulting digest once and trust it as this turn's startup and recovery input.
...
An `ABSENT` marker is meaningful rather than empty: no captain file means this template's defaults, no shared captain file means no inherited preferences, no secondmate file means none registered, and an absent or stale project registry must be rebuilt from the clones before dispatch.
...
The digest's mutating sweeps and its wake-queue drain are already gated on this session actually holding that lock.

Bootstrap detects first, asks for consent, and installs only after the captain approves in the current session.
Do not dispatch until the required tools are present and GitHub authentication is good.
Use `gh-axi` for GitHub, `chrome-devtools-axi` for browser work, and `lavish-axi` for structured decisions or reports; consult current help rather than memorizing flags.
A silent bootstrap section needs no action and `BOOTSTRAP_INFO:` lines are completed facts, but for any other printed diagnostic line, load `bootstrap-diagnostics` and follow its owner procedure.
`secondmate-provisioning` owns startup secondmate sync, liveness, and inherited local-material convergence.

The digest emits exactly one supervision operating block for the detected primary harness.
That emitted block owns the exact wait or wake mechanism, and the script itself never starts supervision.
Treat the digest's status tails as wake-event history rather than current state, and read a task's actual current state with `bin/fm-crew-state.sh <id>` when it matters.
...
Resolve the project independently for every request: an explicit project wins, a clear follow-up inherits its referent, and otherwise match the request against the registry, in-flight work, and project code or README.
Proceed on one confident match while naming the project in plain language; ask one concise question when multiple or no projects plausibly match.
Route by the nature of the work against each registered secondmate scope under section 6, sending in-scope work to the fitting secondmate unless it is blocked or the captain explicitly redirects it.
...
Spawn only through `bin/fm-spawn.sh` after the profile and backend checks in section 4, and a failed worktree-isolation assertion stops the task.
...
Steer a worker with short single-line messages through `fm-send`, and put long instructions in a file.
A secondmate's reply returns through its status or a document pointer, never by reading its chat.
...
Report an autonomous merge to the captain in one line under section 9.
...
A `no-mistakes` ship is ready when the worker reports `done: PR <url> checks green`, and a `direct-PR` ship when it reports `done: PR <url>`.
Run `bin/fm-pr-check.sh <id> <PR url>`, which records the PR in the task's durable record and arms the merge poll.
Give the captain the PR's full URL, a concise outcome summary, and the no-mistakes risk level when applicable, following section 9.
A captain instruction to merge is explicit authority; `yolo` is the only standing routine authority.
A custom `state/<id>.check.sh` must be a mode-`0700` single-link file that prints one line only when firstmate should wake, prints nothing otherwise, and finishes before `FM_CHECK_TIMEOUT`; bind its bytes with `bin/fm-check-register.sh <id>` before the watcher may execute it.

Tear down a ship task only after landing is confirmed, under the unlanded-work rule in section 1.
...
When implementation is separately authorized, promote the existing scout through `bin/fm-promote.sh` rather than creating a duplicate task, and that script owns the carry-over rules the promoted worker must follow.
