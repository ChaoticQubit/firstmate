...
# through bin/fm-tmux-lib.sh, and bin/backends/herdr.sh directly.
#
...
# tail scan). Once an adapter has a candidate
...
# empty|pending|unknown verdict. An adapter that reads a plain (unstyled) screen
# has no ghost styling to strip and relies on the idle-placeholder match
