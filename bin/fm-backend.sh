...
# Callers needing the winning backend read FM_BACKEND_DETECTED after a direct
...
fm_backend_name() {
  ...
  # Called directly (not in a command substitution) so FM_BACKEND_DETECTED
...
#   - jq, for the JSON-emitting experimental adapter (herdr) whose
#     spawn/liveness paths parse the backend's JSON output (see
#     fm_backend_herdr_tool_check);
#   - the treehouse worktree provider, which both backends use.
...
#   exact task id      routed through <state-dir>/<id>.meta's `window=` target -
...
# fm_backend_herdr_composer_state); any other backend reports unknown here -
# callers fall back to their own policy, exactly as an unknown
...
# A third <expected-label> argument is accepted and ignored: callers still pass
# it, and no remaining backend needs it to answer the existence question.
...
# classifier"). Any other backend reports unknown until independently verified.
